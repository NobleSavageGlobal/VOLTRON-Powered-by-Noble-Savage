    ...
    <Card className="p-5 flex flex-col gap-3">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs text-slate-500 capitalize">{client.entity_type}</div>
          </div>
        </div>
        <Badge
          label={client.status}
      ...
      {(client.industry || client.annual_revenue_range) && (
        <div className="flex flex-wrap gap-2 text-xs text-slate-400">
          ...
          {client.annual_revenue_range && <span>💰 {client.annual_revenue_range}</span>}
        </div>
      )}

      <div className="flex items-center justify-between pt-1 border-t border-slate-800">
        <span className="text-xs text-slate-600">Added {formatDate(client.created_at)}</span>
        <button
        ...
        {isLoading ? (
          ...
          <Card className="p-12 text-center">
            ...
            <div className="text-slate-600 text-sm mt-1">
              Add your first client to start tracking financial data, documents, and action plans.
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {clients.map((c) => (
              <ClientCard key={c.id} client={c} onDeleteRequest={setPendingDelete} />
            ))}
          </div>
        )}
      </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-slate-300">
            ...
            <span className="font-semibold text-slate-100">{pendingDelete?.display_name}</span>?
