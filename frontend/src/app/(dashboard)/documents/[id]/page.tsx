...
export default function DocumentDetailPage() {
  ...
  return (
    <div className="flex flex-col flex-1">
      <Header title={doc.original_filename} subtitle={getDocTypeLabel(doc.doc_type)} />
          ...
          <Card>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-slate-200">{getDocTypeLabel(doc.doc_type)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Size</span>
                <span className="text-slate-200">{formatFileSize(doc.file_size)}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-slate-200">{formatDateTime(doc.created_at)}</span>
              </div>
              {doc.processed_at && (
                <div className="flex justify-between">
                  <span className="text-slate-400">Processed</span>
                  <span className="text-slate-200">{formatDateTime(doc.processed_at)}</span>
                </div>
              )}
              {doc.confidence_score !== null && (
                <div>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-slate-200">{Math.round(doc.confidence_score * 100)}%</span>
                  </div>
